...
- FastAPI создает `request_id` и использует его как ключ отслеживания всей команды.
- ESP32 возвращает тот же `request_id` во всех статусах, событиях и ошибках, связанных с этой командой.
- FastAPI отслеживает по `request_id` весь жизненный цикл команды от отправки до финального статуса.
- Ожидаемое время ответа `accepted` от ESP32: 2-3 секунды.
- Ожидаемое время активации реле после принятия команды: 1-2 секунды.
- После открытия ячейки дверь должна оставаться открытой до 30 секунд.
- Если по истечении 30 секунд геркон все еще показывает открытое состояние, это означает, что ячейку забыли закрыть после открытия.
- Для таких случаев FastAPI и ESP32 должны фиксировать отдельное событие или ошибку, связанную с незавершенным закрытием двери.
...
## Правило контроля открытой двери

После события `door_opened` запускается таймер контроля закрытия двери на 30 секунд.

Если за это время геркон не вернулся в состояние закрыто, устройство должно сообщить о нарушении сценария закрытия.
...
```json
{
    ...
    "cell_id": 6,
    "status": "error",
    "error_code": "door_left_open"
}
```

Такой статус нужен, чтобы FastAPI мог отличить штатное открытие от ситуации, когда дверь забыли закрыть после использования.
...
# Статус датчиков DHT11 (пока не реализуем)
...
| door_left_open | Дверь осталась открытой после таймаута контроля |
